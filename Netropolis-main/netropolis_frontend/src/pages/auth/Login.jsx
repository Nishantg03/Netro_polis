...
import { loginRequest, fetchUserProfile, signUpRequest } from "../../features/userFunctions";
...
const Login = () => {
  ...
  const success = async (codeResponse) => {
    ...
    try {
      ...
      let currTokens = await loginRequest({ username, password: pwd });
      // If user doesn't exist yet, auto-register then login
      if (currTokens.error?.message) {
        ...
        const signUpData = await signUpRequest(userProfile);
        if (signUpData.error?.message) {
          throw new Error(signUpData.error.message);
        }
        currTokens = { access: signUpData.access, refresh: signUpData.refresh };
      }
