...
import { signUpRequest, fetchUserProfile, loginRequest } from "../../features/userFunctions";
...
const Register = () => {
    ...
    const success = async (codeResponse) => {
        ...
        try {
            ...
            let currData = await signUpRequest(userProfile);
            // If user already exists, try logging in instead
            if (currData.error?.message) {
                currData = await loginRequest({ username, password: pwd });
                if (currData.error?.message) {
